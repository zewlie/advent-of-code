...
def find_sue_num_2(sue_file, tape_file):
    """Finds the Aunt Sue who meets the requirements specified by the MFCSAM output,
    assuming the following:

        * There must be a greater number of 'cats' and 'trees' than specified
        * There must be a lesser number of pomeranians and goldfish than specified
    """
    ...
    while len(sues) > 1:
        new_sues = {}
        for sue in sues:
            if keys[count] in ('cats', 'trees'):
                if keys[count] not in sues[sue] or reqs[keys[count]] < sues[sue].get(keys[count]):
                    new_sues[sue] = sues[sue]
            elif keys[count] in ('pomeranians', 'goldfish'):
                if keys[count] not in sues[sue] or reqs[keys[count]] > sues[sue].get(keys[count]):
                    new_sues[sue] = sues[sue]
            else:
                if keys[count] not in sues[sue] or reqs[keys[count]] == sues[sue].get(keys[count]):
...
print find_sue_num_2('ex16.txt', 'ex16b.txt')
